...
int printer(int queue) {
    ...
    return length; // Number of printed characters
}
...
void free_process(process_block p[], int id) {
    int beginning = p[id - 1].inst_start, i;
    p[id - 1].state = FINISHED;

    for (i = beginning; i < beginning + p[id - 1].total; i++) {
        bit_vector[i] = 0;
        mem[i] = 0;
    }
...
int allocate_fork(process_block *p, int index_pai) {
    int aux = p[index_pai].total;
    int i, j, k, value, segment = 0;
    int index_inicio, index_medio, index_final;
    ...
    if (aux <= free_space) {
        ...
        p[n_processes].total = aux;

        for (i = 0; i < MEMORY_SIZE; i++) {
            value = bit_vector[i];

            if (value == 0 && segment == 0) { // New Segment
                segment++;
                index_inicio = i;
            } else if (value == 0) { // If it's free and not null
                segment++;
            } else {
                segment = 0;
            }
            // If it's free and null
            if (segment == p[n_processes].total) { // Ideal segment found
                // Allocate
                ...
                free_space -= p[n_processes].total;
...
void allocate_first_fit(process_block *p, int index) {
    int i, j, k, value, segment = 0;
    int index_inicio, index_medio, index_final;

    if (p[index].total <= free_space) {
        for (i = 0; i < MEMORY_SIZE; i++) {
            value = bit_vector[i];

            if (value == 0 && segment == 0) { // novo segmento
                segment++;
                index_inicio = i;
            } else if (value == 0) { // se está livre e segmento não nulo
                segment++;
            } else {
                segment = 0;
            }
            // se está livre e se o segmento é nulo
            if (segment == p[index].total) { // segmento ideal encontrado
                // alocar
