...
int allocate_fork(process_block *p, int index_pai) {
    int i, j, k, segment = -1, index_inicio = -1, index_medio, index_final, process_size = p[index_pai].total, best_size =
            MEMORY_SIZE + 1, best_index = -1;
    ...
    for (i = 0; i < MEMORY_SIZE; i++) {
        if (bit_vector[i] == 0) {
            ...
            if (i == MEMORY_SIZE - 1 && segment >= process_size && segment < best_size) {
                best_size = segment;
                best_index = index_inicio;
            }
        } else {
            if (segment >= process_size && segment < best_size) {
                best_size = segment;
                best_index = index_inicio;
            }
            segment = -1;
    ...
    index_medio = best_index + p[n_processes].n_instructions * 2;
    index_final = best_index + p[n_processes].total;
    p[n_processes].inst_start = best_index;
    p[n_processes].var_start = index_medio;
    p[n_processes].state = READY;
    p[n_processes].index = best_index;

    free_space -= process_size;

    for (j = best_index, k = 0; j < index_medio; j++, k++) {
...
void allocate_best_fit(process_block *p, int index) {
    int i, j, k, segment = -1, index_inicio = -1, index_medio, index_final, process_size = p[index].total, best_size =
            MEMORY_SIZE + 1, best_index = -1;
    ...
    index_medio = best_index + p[index].n_instructions * 2;
    index_final = best_index + p[index].total;
    p[index].inst_start = best_index;
    p[index].var_start = index_medio;
    p[index].state = NEW;
    p[index].index = best_index;
